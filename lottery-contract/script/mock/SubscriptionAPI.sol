...
abstract contract SubscriptionAPI is ConfirmedOwner, IERC677Receiver, IVRFSubscriptionV2Plus {
    ...
    mapping(uint256 => SubscriptionConfig)
        /* subId */ /* subscriptionConfig */ internal s_subscriptionConfigs;
    ...
    function createSubscription() external override nonReentrant returns (uint256 subId) {
        ...
        subId = uint256(
            keccak256(
                abi.encodePacked(
                    msg.sender,
                    blockhash(block.number), // https://github.com/Cyfrin/foundry-full-course-cu/discussions/2246
                    address(this),
                    currentSubNonce
                )
            )
